...
export interface XAxisProps {
  /** Keep static edge labels inset while the plot itself runs full bleed. Default: 0. */
  edgeInset?: number;
  /** Number of ticks to show (including first and last). Default: 5. */
  numTicks?: number;
  /** Show a quiet dot rail at unlabeled data positions. Default: false. */
  showDots?: boolean;
...
const XAxisInner = memo(function XAxisInner({
  ...
  const hoveredLabel =
    isHovering && tooltipData
      ...
      : null;
  const dotPositions = showDots
    ? data
        .map((point) => (xScale(xAccessor(point)) ?? 0) + margin.left)
        .filter(
          (x) =>
            x > edgeInset &&
            x < container.clientWidth - edgeInset &&
            labelsToShow.every((label) => Math.abs(label.x - x) > 20)
        )
    : [];
  ...
  return createPortal(
    <div className="pointer-events-none absolute inset-0">
      {dotPositions.map((x) => (
        <span
          aria-hidden="true"
          className="absolute size-1 -translate-x-1/2 rounded-full bg-chart-label/25"
          key={x}
          style={{ bottom: 18, left: x }}
        />
      ))}
      {labelsToShow.map((item) => (
        <XAxisLabel
          ...
          x={Math.min(
            Math.max(item.x, edgeInset),
            Math.max(edgeInset, container.clientWidth - edgeInset)
          )}
        />
      ))}
    </div>,
