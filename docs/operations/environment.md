...
The two install credentials are exported by `scripts/install-tokens` and
hard-fail when they do not resolve, because `bun install` cannot proceed
without them.

Agent tooling is deliberately outside this contract. `.cursor/install.sh` also
seeds `.agents/skills` from the private agents repo so a cloud agent discovers
the same fleet skill library a local agent gets from the operator's
home-directory links. It reads `CURSOR_CLOUD_AGENTS_GH_READ_TOKEN`, a GitHub
PAT that Cursor injects as an account-level Runtime Secret — no varlock and no
1Password lookup — and every failure path skips seeding rather than failing the
install.
