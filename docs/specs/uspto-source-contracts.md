...
- Bulk-download policy limits one API key to 20 downloads of the same file per year and one IP to five files per 10 seconds. Signed redirects expire after five seconds, so the data-origin request begins immediately and omits the API key. The first file-specific HTTP 429 terminally fails that artifact and preserves safe response metadata without stopping later files. A process-interrupted download becomes terminally failed on restart rather than consuming another same-file request. Global provider failures use persisted bounded backoff.
...
One reconciliation handles one ZIP/XML with fixed projection batches and bounded status-event statements. A successful download is hash-verified and retained independently of projection. Replay deletes only rows still owned by that product and filename before reapplying the retained artifact, with no new provider call. Projection success and failure both keep referenced ZIPs; cleanup deletes only unreferenced orphan objects.
