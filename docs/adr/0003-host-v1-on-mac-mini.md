...
Trademark Turtle runs on the existing Mac mini through corpus bootstrap and the first real users. Its workload is dominated by PostgreSQL with `pg_trgm`, one continuously running pg-boss worker, and one large ZIP/XML working file at a time. The approved Docker Compose topology fits that host directly and avoids a premature multi-service cloud stack.
