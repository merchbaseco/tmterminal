---
summary: Records the decision to host v1 on the Mac mini while preserving a portable move to managed compute, PostgreSQL, and object storage.
read_when:
...
Trademark Turtle runs on the existing Mac mini through corpus bootstrap and the first real users. Its workload is dominated by PostgreSQL with `pg_trgm`, one continuously running pg-boss worker, large ZIP/XML scratch space, retained artifacts, and long replay jobs. The approved Docker Compose topology fits that host directly and avoids a premature multi-service cloud stack.

## v1 boundary

- One Docker host runs the API, worker, one-shot migration, PostgreSQL 16, and Caddy as separate services.
- PostgreSQL and retained artifacts use dedicated volumes. Worker limits prevent bootstrap or replay from starving authenticated queries.
- Database backups leave the Mac mini encrypted; a backup on the same host is not a backup.
- Artifact checksums and inventory are backed up off-host. Automatic artifact deletion is disabled in v1.
...
- The artifact-store interface supports streaming put, get, and head operations. v1 implements a local-volume adapter; an S3-compatible adapter is added only for migration.
...
Do not move only PostgreSQL off the Mac mini while leaving API and worker compute at home. Ingestion traffic, pg-boss, advisory locks, and notifications would cross the home WAN and add latency plus a new ISP failure boundary. Database and compute move together into one region; object storage may move independently first.
...
- authenticated search exceeds 500 ms p95 during ingestion after tuning, sustained host CPU exceeds 70%, memory swaps, or database/artifact disks exceed 70%;
- off-host artifact backup cannot complete within the ingestion interval;
...
The first managed option is a continuous web service and background worker beside managed PostgreSQL with `pg_trgm`, plus S3-compatible artifact storage. Render is the simplest current fit; AWS ECS/Fargate, RDS, and S3 is the higher-control option when its operational surface is justified.

Cloudflare Workers and Vercel Functions may host bounded web work, but they are not the ingestion runtime: worker CPU/duration limits and ephemeral filesystems conflict with large downloads, XML parsing, retained artifacts, pg-boss, and long replay jobs.
